...
public class User {
    ...
    public synchronized void updateXu(int xuUp) throws IOException {
     ...
     public void clanInfoMessage(User us, Message ms) throws IOException {
      ...
      //  log("ID Clan: " + idClan);
        if (clan == 0) {
            // Khong ton tai clan
            ms = new Message(4); DataOutputStream ds = ms.writer();
        ...
        try {
            ...
            red.first();                  //yêu quá :3 cám ơn sếp nha :D ko cần. 5lít nhẹ :) bạc 3 đê :v 10t e có :v còn nhờ a fix vài thứ nữa cơ
            
            DataOutputStream ds;
            if (red.getShort("id")!=0) {
            ms = new Message(117);  ds = ms.writer();
            ...
            ds.writeByte(red.getInt("memmax")); // thành viên limit thằng code php bảo là sv get idicon nên thế
            ...
            Date[] itemClanArray = new Date[11];
            boolean[] isItem = new boolean[11];
            Date DateBayGio = new Date();
            
            JSONObject itemClan = (JSONObject)JSONValue.parse(red.getString("item"));
            int count = 0;
            for(int i = 1; i <= 10; i++) {
                itemClanArray[i] = Until.getDate(itemClan.get(""+i+"").toString());
            ...
            for(int i = 0; i < count; i++) {
                ...
                if (red.first()) {
                    ...
                    ds.writeInt((int)(itemClanArray[i+1].getTime()/1000) - (int)(DateBayGio.getTime()/1000));
                } else {
    ...
    public void clanInfoMember(User us, Message ms) throws IOException {
        Short idClan  = ms.reader().readShort();log("ID Clan: " + idClan);
        if (idClan == +idClan) {
            // Khong ton tai clan
            ms = new Message(4); DataOutputStream ds = ms.writer();
            ds.writeUTF(GameString.ClanIsNull());
        ...
        try {
            ResultSet red;
            red = SQLManager.stat.executeQuery("SELECT * FROM clanmem WHERE clan = '"+idClan+"' LIMIT 1;");
            red.first();
            ...
            ds.writeShort(red.getInt("clan")); // icon clan
            
            ds.writeUTF(red.getString("name")); // tên clan

            ds.writeByte(red.getInt("mem")); // thành viên
            ds.writeByte(red.getInt("memmax")); // thành viên limit 
...
//                                ((Long)jarr.get(j)).shortValue();
               ...
               //red = SQLManager.stat.executeQuery("SELECT * FROM `cms` WHERE `name`='newbaotri' "); 
                 //red.first();
                //.notibaotri = red.getString("content");
                //red.close();    
                ms = new Message(46);
                ds = ms.writer();
                ds.writeUTF(String.format(GameString.newsBaoTri(), us.iddb));
                ds.flush();
                cl.sendMessage(ms);
                // send notification event"SELECT
                red = SQLManager.stat.executeQuery("SELECT * FROM `cms` WHERE `name`='ingame' "); red.first();
                ...
                ms = new Message(46);
                ds = ms.writer();
                ds.writeUTF("Lỗi không đăng nhập thành công, mã lỗi -100");
                ds.flush();
                cl.sendMessage(ms);
    ...
    protected static User registry(ClientEntry cl, String user, String pass) {
        try {
            ...
            if (user.length() < 5) {
                ...
                red.close();
                // Tao tai khoan
                SQLManager.stat.executeUpdate("INSERT INTO user(`user`, `password`, `lock`, `active`) VALUES ('"+user+"', '"+pass+"', 0, 1);");
