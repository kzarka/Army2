 ...
 * @author ASD
 */
public class FireUFO extends Bullet {

    private final short Xboss, Yboss;    
    
    public FireUFO(BulletManager bullMNG, byte bullId, int satThuong, Player pl, int X, int Y, int vx, int vy, int msg, int g100, short Xboss, short Yboss) {
    ...
    public void nextXY() {
       ...
       customXY();
        ...
        if(super.collect)
            this.bullMNG.addBullet(new Bullet(bullMNG, (byte)3, this.satThuong, super.pl, this.Xboss, this.Yboss, 5, 0, 0, 80)); // 230 486
    }
    
     public void customXY() {
         //Số bước nhảy
        frame++;
        ...
        for (int i = 0 ; i < 21; i++)
                
            {
                vxTemp=0;
                  ...
                  //Y = (short)(Y + Math.abs(pl.Y-XmaxY)/4);
