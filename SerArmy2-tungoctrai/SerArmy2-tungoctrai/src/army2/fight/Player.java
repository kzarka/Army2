...
public class Player {
    ...
    public Player(FightManager fightMNG, byte location, short X, short Y, int item[], int teamPoint, User us) throws IOException {
        ...
        this.phongThu  = ability[2]+teamPoint*2;
        //if(this.phongThu >= 2500) this.phongThu = 2000 + teamPoint / 10; //gioi han phong thu
        //this.phongThu  = 0;
    ...
    public final void updateCUP(int addCup) throws IOException {
		...
		if (this.clan > 0) {
                    us.updateClan(0, Math.abs(10 + us.getLevel() / 10));
                        //CÚP lỗi
			//us.updateClan(0, Math.abs(addCup));
    ...
    public void collision(short bx, short by, Bullet bull) {
        ...
        // Tru phong thu
		if (bull.pl.isUsePow && bull.pl.clan > 0 && bull.pl.isItem[5]) {
			dame = dame*105/100;
		}
                ...
                try {
                    ...
                    if(this instanceof Boss) {
                        ...
                        if(idNV == 20)
                            bull.pl.updateEXP(20000);
                        if(idNV == 23){
                            int idQ = Until.nextInt(5000, 10000);
                            bull.pl.updateEXP(5000 + idQ);
                        }
                    } else {
                        int cupCL = bull.pl.us.getDvong()-this.us.getDvong();
                       // int cupAdd = (3000-cupCL)/100;
                        int cupAdd = (cupCL)/100;
