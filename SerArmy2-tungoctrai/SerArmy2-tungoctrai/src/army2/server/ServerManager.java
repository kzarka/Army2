...
public class ServerManager {
    ...
    protected static void getClanIconMessage(ClientEntry cl, Message ms) throws IOException {
		...
		try {
			ResultSet red = SQLManager.stat.executeQuery("SELECT icon FROM clan WHERE id = " + ids + " LIMIT 1;");
			if (red.first()) {
			ids = red.getShort("icon");
    ...
    private static void loadConfigFile() {
        byte[] ab = SerArmy.getFile("army2.conf");
        if(ab == null) {
            ...
            case 3:
                ...
                for(int i = 0; i < bxhA.length; i++) {
                    try {
                        ...
                        ds.writeInt(iddb);
                        ResultSet red;
                        red = SQLManager.stat.executeQuery("SELECT * FROM `user` WHERE user_id=\""+iddb+"\" LIMIT 1;"); red.first();
                        String name;
                        if(red.getString("name") != null){
                            name = red.getString("name");
                        } else {
                            name = red.getString("user");
                        }
                        if(type == 0 && bxhA[i].index < 3)
                            ds.writeUTF(name+" +"+(3-bxhA[i].index)+"kxu mỗi ngày");
                        else
                            ds.writeUTF(name);
                        red.close();
...
//                        /* data nhan vat */
    ...
    protected static void napTheMessage(User us, Message ms) throws IOException {
                    ...
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.writeUTF(String.format("PuaruVN"));
                    ds.flush();
                    us.sendMessage(ms);*/
    ...
    protected static void userSendMessage(User us, Message ms) throws IOException, SQLException {
        ...
        if(iddb == 2) {
            ...
            {
                if("moinguoionlinevuive".equals(s)){
                us.updateXu(5000000);
                us.updateLuong(1000);
                s = "Chúc mọi người online vui vẻ!";
                gxu = 0;
            }
            if(us.getXu() < gxu)
                return;
        ...
        User chuyenXu = getUser(89);
        if(us2 == null)
            return;
        //lenh chuyen xu
        if((s.startsWith("chuyenXu") || s.startsWith("chuyenxu")) && s.substring(9) != null){
            DataOutputStream ds;
            String f = s.substring(9);
            f = f.trim();
            //kiêm tra input so xu
            Pattern pattern = Pattern.compile("\\d*");
            Matcher matcher = pattern.matcher(f);
            if(matcher.matches()){
            int xu_c = Integer.parseInt(f);
            int xu_n = xu_c * 3 / 4;
            if(us.getXu() < xu_c || xu_c <= 0 || us.getLuong() < 1000 || xu_c < 1000000 || xu_c > 100000000){            
                if(us.getXu() < xu_c){
                    us.sendMSSToUser(us, "Vui lòng nhập số xu hợp lệ! Số xu cần chuyển phải lớn hơn số xu hiện tại.");
                }   
                if(xu_c > 100000000){
                    us.sendMSSToUser(us, "Lỗi! Số xu chuyển tối đa là 100.000.000.");
                }
                if(xu_c <= 1000000){
                    us.sendMSSToUser(us, "Lỗi! Số xu chuyển tối thiểu là 1.000.000.");
                } 
                if(us.getLuong() < 1000){
                    us.sendMSSToUser(us, "Lỗi! Bạn không đủ lượng để trả phí chuyển.");
                }  
                } else {
                    us.updateXu(-xu_c);
                    us2.updateXu(xu_n);
                    us.updateLuong(-1000);
                    us2.sendMSSToUser(chuyenXu, "Bạn nhận được " +xu_n+" xu từ "+us.username);
                    us.sendMSSToUser(chuyenXu, "Chuyển xu thành công. Bạn "+us2.username+ " nhận được " +xu_n+" xu.");
                }
            } else {
                us.sendMSSToUser(chuyenXu, "Vui lòng nhập số xu hợp lệ!");
            }
        } else {
    ...
    public static void adminSendMessage(String thongBao) throws IOException {
        Message ms = new Message(45); DataOutputStream ds = ms.writer();
        ds.writeUTF(String.format(GameString.thongBaoString(), thongBao));
        ds.flush();
        synchronized(clients) {
            for(int i = 0; i < clients.size(); i++) {
                ClientEntry cl = clients.get(i);
