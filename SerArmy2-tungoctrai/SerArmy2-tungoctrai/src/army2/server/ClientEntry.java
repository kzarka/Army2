...
public class ClientEntry implements ISession {
    ...
    class MessageCollector implements Runnable {
        ...
        private Message readMessage() throws Exception {
            ...
            /*if (cmd != 126)
            {
                log("Có cái lồn đòi restart địt mẹ mày!!");
            }
            else {
                ServerManager.adminSendMessage("Hệ thống sẽ bảo trì sau 5 phút nữa !!!");
                Thread.sleep(4 * 1000 * 60);
                
                ServerManager.adminSendMessage("Hệ thống sẽ bảo trì sau 1 phút nữa !!!");
                ...
                ServerManager.init();
                // Set online to 0
                SQLManager.stat.executeUpdate("UPDATE `armymem` SET `online`=0, `idapp`=-1");
                ServerManager.start();
            }*/
...
//            recvByteCount += (5 + byteRead);
    ...
    private static final byte[] key = "gauk0lkhum".getBytes();
    public Socket sc;
...
//                sendByteCount += 5;
    ...
    public void loginMessage(Message ms) throws IOException {
        ...
        System.out.println("Client: "+id+" User: "+userS+ " Version: "+version);
