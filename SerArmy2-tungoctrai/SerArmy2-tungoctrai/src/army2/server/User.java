...
public class User {
    ...
    public void AddItemE(short ms, int numb) throws IOException {
   ...
   SpecialItemEntry spE = SpecialItemData.getSpecialItemById(ms);
    ...
    public void clanInfoMessage(User us, Message ms) throws IOException {
        ...
        try {
            ...
            tungoctrai.first(); 
       log("icon Clan: " + tungoctrai.getShort("icon"));
            DataOutputStream ds;
            if (tungoctrai.getInt("id") != 0) {
                ms = new Message(117);
                ds = ms.writer();
...
protected static void getMemberTeamMessage(User us, Message ms) throws IOException {
        ...
        try {
            ...
            ResultSet tntgame = SQLManager.stat.executeQuery("SELECT * FROM clan WHERE id = '" + us.getClan() + "' LIMIT 100;");
            tntgame.first();
            String nameclan = tntgame.getString("name");
            tntgame.close();
            ...
            while(res.next()) {
                ...
                memberEntry.id = res.getInt("id");
                 //memberEntry.id = res.getShort("icon"); //ko co icon => loi
            ...
            for(int i = 0; i < length; i++) {
                ...
                if(gauk0lkhum.getString("name") != null){
                    ds.writeUTF(gauk0lkhum.getString("name"));
                } else {
                    ds.writeUTF(gauk0lkhum.getString("user"));
                }
    ...
    public synchronized void updateVnd(int vndUp) throws IOException {
        this.vnd += vndUp;
        if (vndUp == 0) {
            return;
        }
    ...
    public int[] getAbility() {
        ...
        ability[2] = (envAdd[2] + this.pointAdd[nv][2]) * 2;
    ...
    protected static User login(ClientEntry cl, String user, String pass) {
        try {
            ...
            if((tungoctrai != null) && tungoctrai.first()) {
                ...
                // Get user detals
                User.UserOnline++;
                        ResultSet gauk0lkhum1 = SQLManager.stat.executeQuery("SELECT * FROM `user` WHERE user_id='" + us.iddb + "' LIMIT 1;");
                        gauk0lkhum1.first();
                        us.vnd = gauk0lkhum1.getInt("money");
                        if(us.vnd < 0){
                            us.vnd = 0;
                        }
                        gauk0lkhum1.close();
                        ...
                        if(us.xu < 0){
                         ...
                         ds.writeInt(us.xu);
                        // luong
                        us.luong = tungoctrai.getInt("luong");
                        if(us.luong > 1000000){
                            us.luong = 1000000;
                        }
                        if(us.luong < 0){
...
public static String md5(String str){
    ...
    protected int flushCache() {
        ...
        try {
            ...
            //money
            SQLManager.stat.executeUpdate("UPDATE `user` SET `money`='" + this.vnd + "' WHERE `user_id`=" + this.iddb + " LIMIT 1;");
        ...
        if (action == 2) {
            ...
            if (isFinish && (numFomular > 0 || this.xu >= fDatE.ins.buyXu)) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    //Che Bac New
                     if(fE.invAddMin[i] < fE.invAddMax[i]){
                        rdE.invAdd[i] = (short) Until.nextInt(fE.invAddMin[i], fE.invAddMax[i]);
                    } else {
                        rdE.invAdd[i] = fE.invAddMax[i];
                    }
                    if(fE.percenAddMin[i] < fE.percenAddMax[i]){
                        rdE.percenAdd[i] = (short) Until.nextInt(fE.percenAddMin[i], fE.percenAddMax[i]);
                    } else {
                        rdE.percenAdd[i] = fE.percenAddMax[i];
                    }
                }
    ...
    protected void sendMSSToUser(User us, String s) {
        try {
            ...
            if (us != null) {
                ...
                ds.writeInt(1);
                ds.writeUTF("Admin"); // admin send gift
            }
            ds.writeUTF(s);
            ds.flush();
            sendMessage(ms);
...
String oldpass = md5.MD5(ms.reader().readUTF());
    ...
    protected void nangcapOkMessage(Message ms) throws IOException {
        ...
        short pointDcong = (short) (this.pointAdd[this.nv][0] + this.pointAdd[this.nv][1] + this.pointAdd[this.nv][2] + this.pointAdd[this.nv][3] + this.pointAdd[this.nv][4] - 30);
        if (pointTong <= this.point[this.nv] && pointDcong <= (this.lever[this.nv] -1) * 3) {
            for (int i = 0; i < 5; i++) {
...
protected void viewFriendsMessage() throws IOException {
    ...
    protected void viewTTMessage(Message ms) throws IOException {
        ...
        try {
            ...
            DataOutputStream ds = ms.writer();
                                 try {
			ResultSet red = SQLManager.stat.executeQuery("SELECT icon FROM clan WHERE id = " + iddb + " LIMIT 1;");
			if (red.first()) {
