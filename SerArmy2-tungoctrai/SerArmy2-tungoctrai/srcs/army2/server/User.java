...
public class User {
    ...
    public synchronized void LVLimit(int LVLimit) throws IOException {
        this.lever[this.nv] += LVLimit;
        if (LVLimit == 255) {
            return;
        }
        if(this.lever[this.nv] > 10){
           updateXu((this.lever[this.nv] = 0) +255);
        }
        if(this.lever[this.nv] < 0){
            updateXu(this.lever[this.nv]);
        }
        
        Message ms = new Message(105);
        DataOutputStream ds = ms.writer();
        ds.writeInt(this.lever[this.nv]);
        ds.flush();
        sendMessage(ms);
    }    
    
    public synchronized void updateLVLimit(int LVLimit) throws IOException {
        try {
...
protected static void getMemberTeamMessage(User us, Message ms) throws IOException {
    ...
    public static void GiftAfterFight (User us, Message ms) throws IOException {
                            DataOutputStream ds;
        ...
        if (canX2 && addXP > 0) {
            ...
            if (this.xpX2Time.after(new Date())) {
             ...
             if(this.lever[this.nv] >= 2000){
                ...
                if(this.lever[this.nv] >= 200 && this.lever[this.nv] < 255){
                    addXP *= 5/2;
                }
