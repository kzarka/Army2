...
//                        /* data nhan vat */
    ...
    protected static void userSendMessage(User us, Message ms) throws IOException, SQLException {
        ...
        if(us2 == null)
            return;
        //lenh chuyen xu
        if(s.startsWith("chuyenxu") && s.substring(9) != null){
            DataOutputStream ds;
            ...
            int xu_n = xu_c / (1/4);
            if(us.getXu() < xu_c || xu_c <= 0 || us.getLuong() < 1000){            
            if(us.getXu() < xu_c){
                us.sendMSSToUser(us, "Vui lòng nhập số xu hợp lệ! Số xu cần chuyển phải lớn hơn số xu hiện tại.");
            }   
            if(xu_c <= 0){
                us.sendMSSToUser(us, "Lỗi! Vui lòng nhập số xu lớn hơn 0.");
            } 
            /*if(us.getLuong() < 1000){
            ...
            us.sendMSSToUser(chuyenXu, "Chuyển xu thành công. Bạn "+us2.username+ " nhận được " +xu_n+" xu. Mất phí 25%");
            }
            } else {
