...
public class FightManager {
    ...
    private void nextBoss() throws IOException {
        ...
        if(this.wait.map == 34) {
            short X = (short)(880);
            short Y = (short)(50);
            players[allCount] = new TRex(this, (byte)15, "T-Rex", (byte)allCount, 50000, X, Y);
            allCount++;
            int numBoss = new int[] {0, 0, 1, 1, 1, 1, 1, 1, 1}[playerCount];
            for(int i = 0; i < numBoss; i++) {
        ...
        //KV cam
            ...
            int numBoss = (new int[]{4, 4, 5, 5, 6, 8, 8, 9, 9})[playerCount];
            for(int i = 0; i < numBoss; i++) {
                short X = (short)((i%2 == 0)? Until.nextInt(48, 231):Until.nextInt(950, 1000));
                short Y = (short)(50+40*Until.nextInt(3));
                players[allCount] = new HiMaLapSon(this, (byte)17, "BOSS", (byte)allCount, 10000, X, Y);
                allCount++;
            }
        }
            ...
            for(int i = 0; i < numBoss; i++) {
                short X = (short)((i%2 == 0)? Until.nextInt(100, 200):Until.nextInt(950, 1000));
                short Y = (short)(50+40*Until.nextInt(3));
                players[allCount] = new HangNhen(this, (byte)18, "BOSS", (byte)allCount, 10000, X, Y);
                allCount++;
            }
        }              
        ...
        for(int i = 0; i < ServerManager.maxPlayers; i++) {
            ...
            int abc = rd.nextInt(4);
            {
               if(abc == 2)
               {
                   ...
                   short itemhopqua = 95;
                    if(abcd == 0) itemhopqua = 84;
                    if(abcd == 3) itemhopqua = 85;                   
                    if(abcd == 0) itemhopqua = 99;
                    if(abcd == 3) itemhopqua = 94;
                    if(abcd == 4) itemhopqua = 95;
                    if(abcd == 5) itemhopqua = 96;
                    if(abcd == 6) itemhopqua = 97;
                   pl.us.AddItemE(itemhopqua);
               }
            }
        ...
        if(this.wait.money > 0) {
            for(int i = 0; i < ServerManager.maxPlayers; i++) {
                ...
                if(win >= 0) {
                    ...
                    ds.flush();
                    sendToTeam(ms);
                            ms = new Message(119); ds = ms.writer();
                            ds.writeByte(0);
                            ds.writeUTF("Bạn nhận được 2 lượt chọn quà!");
                            ds.flush();
                            sendToTeam(ms);
                }
                /*else if (win ==1) {
...
public void newShootBoss(int index, byte bullId, short arg, byte force, byte force2, byte nshoot, boolean ltap, short Xboss, short Yboss) throws IOException {
    ...
    public void TRexSaid(Player pl) throws IOException {
        Message ms = new Message(9); DataOutputStream ds = ms.writer();
        ds.writeInt(0);
        ds.writeUTF("TRex: Hãy nhận những phát bắn của ta, địt mẹ mày!");
        ds.flush();
        this.sendToTeam(ms);
    }    
