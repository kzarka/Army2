...
protected static void getMemberTeamMessage(User us, Message ms) throws IOException {
        ...
        try {
            ...
            ds.writeUTF("By TrungKenbi"); // tên đội
        ...
        try {
            ...
            if ((red != null) && red.first()) {
                ...
                if (!passcode) {
                    Message ms = new Message(4);
                    DataOutputStream ds = ms.writer();
                    ds.writeUTF(String.format(GameString.userLoginPassCode(), us.username, us.report));
                    ds.flush();
                    cl.sendMessage(ms);
...
//                                ((Long)jarr.get(j)).shortValue();
    ...
    protected static User registry(ClientEntry cl, String user, String pass) {
        try {
            Message ms = new Message(4);
                DataOutputStream ds = ms.writer();
                ds.writeUTF("Truy cập http://army.tntgames.info để đăng ký");
                ds.flush();
                cl.sendMessage(ms);
            /*
            User us = new User(cl);
