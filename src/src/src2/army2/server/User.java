...
public class User {
    ...
    public Date getCanChat() throws SQLException {
        ResultSet red = SQLManager.stat.executeQuery("SELECT `canChat` FROM armymem WHERE id = " + iddb);
        red.first();
        Date date = Until.getDate(red.getString("canChat"));
        red.close();
        return date;
    }
    ...
    public void clanInfoMessage(User us, Message ms) throws IOException {
        ...
        try {
            ...
            if (red.getShort("id") != 0) {
                ...
                ds.writeShort(red.getShort("icon")); // icon clan
...
protected static void getMemberTeamMessage(User us, Message ms) throws IOException {
        ...
        try {
            ...
            log("count-1: " + red.getRow());
            red.close();

            ds.writeUTF("Đức Đẹp Zai"); // tên đội
    ...
    /*
        Thêm vật phẩm đặc biệt vào rương đồ
    */
    public synchronized void addGift(int id, int numb) throws IOException {
    ...
    public synchronized void updateXP(int addXP, boolean canX2) throws IOException {
        if (canX2 && addXP > 0) {
            ...
            if(hours >= 17 && hours <= 22) // X2 Exp từ 7h tối tới 11h tối
            {
                addXP *= 6;
           }
        ...
        try {
            ...
            if(hours == 5 && minutes < 30) // Bảo trì
            {
                Message ms = new Message(4);
                    DataOutputStream ds = ms.writer();
                    ds.writeUTF("Server đang bảo trì, mời bạn quay lại sau 30p nữa");
                    ds.flush();
                    cl.sendMessage(ms);
                    Thread.sleep(10000);
                    cl.close();
            ...
            if ((red != null) && red.first()) {
                ...
                int permission = red.getInt("permission");
                ...
                //permission khac 0 moi co the vao
                /*else if (permission == 0) {
                    Message ms = new Message(4);
                    DataOutputStream ds = ms.writer();
                    
                    ds.writeUTF(String.format(GameString.loginServerTest()));
                    ds.flush();
                    cl.sendMessage(ms);
                    Thread.sleep(10000);
                    cl.close();
...
//                                ((Long)jarr.get(j)).shortValue();
                    ...
                    if (Until.getNumDay(lastLogin, new Date()) >= 1) {
                        ...
                        us.updateXP(xp, true);
                        us.sendMSSToUser(null, String.format(GameString.quaOffline(), xu, luong, xp));
                    }
            ...
            if(hourss >= 17 && hourss <= 23) // X2 Exp từ 7h tối tới 11h tối
            {
                ms = new Message(46);
                    ds = ms.writer();
                    ds.writeUTF(String.format("X12 Toàn Sever Từ 17h Tới 22h Hết. Chúc Ae onl vv!", us.username));
                    ds.flush();
                    cl.sendMessage(ms);
            }
                    // send news event
                    ms = new Message(46);
                    ds = ms.writer();
                    ds.writeUTF(String.format("Tài khoản mới sau khi đăng nhập vào shop mua x2 đợi giờ x12 rồi sử dụng Rương Exp để nhận thêm kinh nghiệm nha! ", us.username));
                    ds.flush();
                    cl.sendMessage(ms);
                    //noti update
                    ms = new Message(46);
                ...
                ms = new Message(46);
                ds = ms.writer();
                ds.writeUTF("Đăng nhập thất bại, vui lòng kiểm tra mật khẩu và tài khoản hoặc trưa đăng kí thì zô Army2lau.ga để đăng kí nhé!");
                ds.flush();
                cl.sendMessage(ms);
    ...
    public void setCanChat() {
        try {
            SQLManager.stat.executeUpdate("UPDATE armymem set canChat = ADDTIME(now(),'00:00:00') where id = " + iddb);
        } catch (SQLException ex) {
            Logger.getLogger(User.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    protected static User registry(ClientEntry cl, String user, String pass) {
        try {
            Message ms = new Message(4);
                DataOutputStream ds = ms.writer();
                ds.writeUTF("Truy cập http://army2lau.ga để đăng ký");
                ds.flush();
                cl.sendMessage(ms);
            /*
            User us = new User(cl);
    ...
    protected void hopNgocMessage(Message ms) throws IOException, SQLException {
        ...
        if (action == 0) {
            ...
            if (hopNgocTB == null && hopNgocItemArray.size() == 1) {
                ...
                if ((rdE.entry.id >= 50) && (rdE.entry.id != 91)&& (rdE.entry.id != 92) && (rdE.entry.id != 93)&& (rdE.entry.id != 94)&& (rdE.entry.id != 95)){
                    if (rdE.entry.id == 50) {
                    ...
                    if (rdE.entry.id < 106) {
                        ...
                        ds = ms.writer();
                        ds.writeByte(0);
                        ds.writeUTF("Bạn muốn sử dụng nó không?");
                        ds.flush();
                        sendMessage(ms);
                        return;
                    }
                } else if (((rdE.entry.id + 1) % 10 != 0) && rdE.numb >= 5 && rdE.entry.id != 91 && rdE.entry.id != 92 && rdE.entry.id != 93 && rdE.entry.id != 94 && rdE.entry.id != 95) {
        ...
        if (action == 1) {
            ...
            if (this.hopNgocAction == 2) {
                ...
                if (rdE.entry.id == 50) {
                    ...
                    this.updateXP(10000000,true);
                    updateRuong(null, null, -1, null, hopNgocItemArray);
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Nhận 10tr exp thành công vào shop mua x2 và đợi giờ x12 rồi sử dụng để nhận X8 exp nhé! ");
                    ds.flush();
                    sendMessage(ms);
                    return;
                }
                     ...
                     this.addGift(86, 1);                                            
                    updateRuong(null, null, -5, null, hopNgocItemArray);
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Chúc mừng bạn đã nhận công bánh trưng hoàn thiện");
                    ds.flush();
                    sendMessage(ms);
                    return;
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Chúc mừng bạn đã nhận thành công bánh tét hoàn thiện");
                    ds.flush();
                    sendMessage(ms);
                    return;
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Sử dụng ăn con cặc à xoạc toét lone ny mày bây giờ -.-");
                    ds.flush();
                    sendMessage(ms);
                    return;
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Nhận 500 lượng thành công");
                    ds.flush();
                    sendMessage(ms);
                    return;
                } else if(rdE.entry.id == 1000){
                   ...
                   this.point[this.nv] = (short)(100);
                     updateRuong(null, null, -1, null, hopNgocItemArray);                  
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Nhận 100 Point thành công");
                    ds.flush();
                    sendMessage(ms);
                    return;
                }
                else if(rdE.entry.id == 888){
                    int i = rd.nextInt(6);
                    int exp = 3;
                    if(i == 0) exp = 3;
                    if(i == 1) exp = 6;
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Nhận thành công "+exp+" kinh nghiệm và Bánh Tét");
                    ds.flush();
                    sendMessage(ms);
                    return;
                }
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Nhận thành công "+exp+" kinh ngiệm và Bánh Trưng");
                    ds.flush();
                    sendMessage(ms);
                    return;
                }
                    ...
                    String name="Triệu lệnh point";
                    short IDItem=98;
                    if(rd == 1) 
                    {
                        name = "Zêzê bạn đã mở dc thạch anh tím siêu cấp 2"; IDItem = 41;
                    }
                    if(rd == 2) 
                    {
                        name = "Rương Xp"; IDItem = 102;
                    }
                    if(rd == 3) 
                    {
                        name = "Zêzê bạn đã mở dc thạch anh tím siêu cấp 2"; IDItem = 41;
                    ...
                    if(rd == 4) 
                    {
                        name = "Chúc mừng bạn nhận đc thạch anh tím cấp 999"; IDItem = 40;
                    }
                    if(rd == 6) 
                    {
                        name = "Zêzê bạn đã mở dc thạch anh tím siêu cấp 2"; IDItem = 41;
                    ...
                    String name="Rương Lượng";
                    short IDItem=97;
                    int Xu = 0;
                    int Luong = 0;
                    if(rd == 0) 
                    {
                        name = "Cu đen lắm"; IDItem = 0;
                    }
                    if(rd == 1) 
                    {
                    ...
                    if(rd == 4) 
                    {
                        name = "10.000.000 xu và Rương lương"; Xu = 20000000;
                    }
                    if(rd == 5) 
                    {
                        name = "1.000 lượng và Rương lượng"; Luong = 10000;
                    }
                    if(rd == 6) 
                    {
                        name = "Cu đen lắm"; IDItem = 0;
                    ...
                    if(rd == 7) 
                    {
                        name = "Ngọc Thạch Anh Tím cấp 11"; IDItem = 95;
                    }
                    this.AddItemE((short)IDItem);
                    updateXu(Xu);
                    updateLuong(Luong);
                    ...
                    if(rd == 0) 
                    {
                        name = "Chúc mừng bạn nhận đựoc ngọc siêu cấp 1 :v"; IDItem = 0;
                    }
                    if(rd == 1) 
                    {
                        name = "Hồng Ngoc cấp 11"; IDItem = 93;
                    ...
                    if(rd == 2) 
                    {
                        name = "Chúc mừng cu được ngọc siêu cấp 1 :v"; IDItem = 0;
                    }
                    if(rd == 3) 
                    {
                        name = "Chúc mừng cu nhận được ngọc siêu cấp 1 :v"; IDItem = 0;
                    }
                    if(rd == 4) 
                    {
                        name = "5.000.000 xu và Rương lương"; Xu = 5000000;
                    }
                    if(rd == 5) 
                    {
                        name = "200 lượng và Rương lượng"; Luong = 200;
                    }
                    if(rd == 6) 
                    {
                        name = "Chúc mừng cu nhận được ngọc siêu cấp 1 :v"; IDItem = 0;
            ...
            sendMessage(ms);
                ...
                else if(rdE.entry.id == 106){
                    int xu = Math.abs(this.xu);
                    if(this.xu > 1){
                    updateXu(xu*1);
                    updateRuong(null, null, -1, null, hopNgocItemArray);
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Đã fix âm thành công số xu hiện tại của bạn là: "+xu+"xu.");
                    ds.flush();
                    sendMessage(ms);
                    return;
                    } else {
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Bạn không bị âm xu!");
                    ds.flush();
                    sendMessage(ms);
                    }
                }
...
public void AddItemE(short ms) throws IOException {
   ...
   SpecialItemEntry spE = SpecialItemData.getSpecialItemById(ms);
    ...
    public void viewFriendsMessage() throws IOException {
        try {
            ...
            for (int i = 0; i < jarr.size(); i++) {
                ...
                ds.writeInt(iddbFR);
                red = SQLManager.stat.executeQuery("SELECT user FROM user WHERE user_id=\"" + iddbFR + "\" LIMIT 1;");
                red.first();
                ...
                for (int j = 0; j < 5; j++) {
                    short indexS = Short.parseShort(jarr2.get(j).toString());
