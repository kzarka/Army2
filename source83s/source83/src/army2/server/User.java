...
//                                ((Long)jarr.get(j)).shortValue();
    ...
    public void setCanChat() {
        try {
            SQLManager.stat.executeUpdate("UPDATE armymem set canChat = ADDTIME(now(),'00:05:00') where id = " + iddb);
        } catch (SQLException ex) {
            Logger.getLogger(User.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
        ...
        if (action == 1) {
            ...
            if (this.hopNgocAction == 2) {
                ...
                if (rdE.entry.id == 50) {
                    ...
                    ms = new Message(45);
                    ds = ms.writer();
                    ds.writeUTF("Nhận 1000 lượng thành công");
                    ds.flush();
                    sendMessage(ms);
                    return;
    ...
    protected void sendMSSToUser(User us, String s) {
        try {
            ...
            if (us != null) {
                ...
                ds.writeInt(1);
                ds.writeUTF("Berus"); // để cái này nha :V
            }
            ds.writeUTF(s);
            ds.flush();
            sendMessage(ms);
